...
: rclcpp::Node("wifi_monitor")
{
  ...
  wifi_diagnostics_topic_ =
    declare_parameter<std::string>("wifi_diagnostics_topic", wifi_diagnostics_topic_);

  wifi_diagnostics_pub_ = create_publisher<diagnostic_msgs::msg::DiagnosticArray>(
    wifi_diagnostics_topic_, rclcpp::QoS(1).best_effort());
  ...
  wifi_diagnostics_timer_ =
    create_wall_timer(std::chrono::milliseconds(period_ms_), [this] {tick();});
}
...
std::optional<std::pair<double, double>> WifiMonitor::read_wireless()
{
  ...
  while (std::getline(f, line)) {
    ...
    if (
      std::sscanf(line.c_str() + pos, "%*[^:]: %lf %lf %lf %lf", &status, &linkv, &level, &noise) >=
      3)
    {
...
double WifiMonitor::ema_step(
  const double & sample, const double & alpha, std::optional<double> & state)
{
  if (!state.has_value()) {
